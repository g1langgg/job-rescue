...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const buttons = document.querySelectorAll('.btn-primary, .btn-register, .btn-contact, .btn-view-profile, .btn-load-more, .btn-filter, .btn-reset, .btn-post-job');
    ...
    function addClickHandlers() {
        ...
        testimonialCards.forEach(card => {
            card.addEventListener('click', function() {
                const authorName = this.querySelector('.author-info h4').textContent;
                showTestimonialModal(authorName);
            });
        });
        ...
        socialLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                if (platform.includes('facebook')) {
                    ...
                    shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent('Temukan pekerjaan mikro dan UMKM di JobRescue Bogor!')}`;
                } else if (platform.includes('linkedin')) {
                    shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}`;
    ...
    // Modal functions
    function showCategoryModal(categoryName) {
        showModal(`Kategori: ${categoryName}`, `Jelajahi ribuan pekerjaan dalam kategori ${categoryName}. Temukan peluang yang sesuai dengan keahlian Anda.`);
    }
    
    function showFeatureModal(featureTitle) {
        showModal(`Fitur: ${featureTitle}`, `Pelajari lebih lanjut tentang fitur ${featureTitle} yang akan membantu Anda dalam mencari pekerjaan atau talent.`);
    }
    
    function showStepModal(stepTitle) {
        showModal(`Langkah: ${stepTitle}`, `Ikuti langkah ${stepTitle} untuk memulai perjalanan karir Anda dengan JobRescue Bogor.`);
    }
    
    function showTestimonialModal(authorName) {
        showModal(`Testimoni dari ${authorName}`, `Baca pengalaman lengkap ${authorName} dalam menggunakan JobRescue Bogor untuk menemukan pekerjaan impian.`);
    }
    ...
    function showModal(title, content) {
        ...
        modalContent.innerHTML = `
            ...
            <p style="margin-bottom: 2rem; opacity: 0.8; line-height: 1.6;">${content}</p>
            <button onclick="this.closest('div').remove()" 
                    style="background: linear-gradient(135deg, #f97316, #ea580c); color: white; border: none; padding: 1rem 2rem; border-radius: 50px; cursor: pointer; font-weight: 600;">
                Tutup
            </button>
